...
namespace :spec do
  ...
  task :deps do
    sh "#{Bundler::GemHelper.gem_cmd} list ronn | (grep 'ronn' 1> /dev/null) || #{Bundler::GemHelper.gem_cmd} install ronn --no-ri --no-rdoc"
    sh "#{Bundler::GemHelper.gem_cmd} list rspec | (grep 'rspec (2.' 1> /dev/null) || #{Bundler::GemHelper.gem_cmd} install rspec --no-ri --no-rdoc"
  end
end
...
begin
  ...
  begin
    ...
    namespace :ci do
      ...
      task :deps do
        sh "#{Bundler::GemHelper.gem_cmd} list ci_reporter | (grep 'ci_reporter' 1> /dev/null) || #{Bundler::GemHelper.gem_cmd} install ci_reporter --no-ri --no-rdoc"
      end
      task :deps => "spec:deps"
    end
  end
