  ...
  context "with a standard Gemfile" do
    ...
    context "when show command deprecation is enabled" do
      ...
      it "prints the path to the running bundler" do
        bundle "show bundler"
        expect(out).to eq(
          "[DEPRECATED] use `bundle info bundler` instead of `bundle show bundler`\n" +
          root.to_s
        )
      end
      ...
      it "prints path if gem exists in bundle (with --paths option)" do
        bundle "show rails --paths"
        expect(out).to eq(
          "[DEPRECATED] use `bundle info rails --path` instead of `bundle show rails --paths`\n" +
          default_bundle_path("gems", "rails-2.3.2").to_s
        )
      end
      ...
      it "prints path of all gems in bundle sorted by name" do
        ...
        out_lines = out.split("\n")
        expect(out_lines[0]).to eq("[DEPRECATED] use `bundle list` instead of `bundle show --paths`")
