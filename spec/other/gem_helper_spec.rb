...
describe "Bundler::GemHelper tasks" do
  ...
  context "version management" do
    def version_file
      File.readlines version_filename
    end
    ...
    it "increases the patch by one" do
      ...
      gemspec_path = File.join(app, 'test.gemspec')
      gemspec = Bundler.load_gemspec gemspec_path
      gemspec.version.to_s.should == "0.0.2"
    end
    ...
    #  new_file = File.read(File.join(app.to_s, 'lib', 'test', 'version.rb'))
    #  p new_file
    #  new_file.should match /3\.5\.8/
    #end
