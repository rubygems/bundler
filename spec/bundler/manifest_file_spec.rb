...
describe "Bundler::Environment" do
  ...
  it "finds the default manifest file" do
    build_manifest_file
    Dir.chdir(tmp_dir)
    Bundler::Environment.load.filename.should == tmp_file("Gemfile")
  end
  ...
  it "sets the default bundle path to vendor/gems" do
    build_manifest_file
    Dir.chdir(tmp_dir)
    Bundler::Environment.load.gem_path.should == tmp_file("vendor", "gems")
  end
  ...
  it "allows setting the bundle path in the manifest file" do
    ...
    Gemfile
    Dir.chdir(tmp_dir)
    Bundler::Environment.load.gem_path.should == tmp_file("gems")
  end
  ...
  it "assumes the bundle_path is relative to the manifest file no matter what the current working dir is" do
    ...
    Bundler::Environment.load.gem_path.should == tmp_file('..', 'cheezeburgerz')
  end
  ...
  it "sets the default bindir relative to the Gemfile" do
    ...
    FileUtils.mkdir_p(tmp_file("wot"))
    Dir.chdir(tmp_file("wot"))
    Bundler::Environment.load.bindir.should == tmp_file("bin")
  end
  ...
  it "allows setting the bindir in the manifest file" do
    ...
    Gemfile
    Dir.chdir(tmp_dir)
    Bundler::Environment.load.bindir.should == tmp_file('binz')
  end
  ...
  it "assumes the bindir is relative to the manifest file no matter what the current working dir is" do
    ...
    FileUtils.mkdir_p(tmp_file('w0t'))
    Dir.chdir(tmp_file('w0t'))
    Bundler::Environment.load.bindir.should == tmp_file('..', 'cheezeburgerz')
  end
