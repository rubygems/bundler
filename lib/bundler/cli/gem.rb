  ...
  class CLI::Gem
    ...
    def run
      ...
      if test_framework = ask_and_set_test_framework
        ...
        when "test-unit"
          dir = namespaced_path.include?("/") ? File.dirname(namespaced_path) : ""
          file = File.basename(namespaced_path)
          templates.merge!(
            "test-unit/helper.rb.tt" => "test/helper.rb",
            "test-unit/newgem_test.rb.tt" => File.join("test", dir, "test_#{file}.rb")
          )
        end
      end

      config[:test_task] = /test/ =~ config[:test] ? "test" : "spec"
