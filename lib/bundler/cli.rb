...
module Bundler
  class CLI < Thor
    ...
    method_option :local, :type => :boolean, :default => false, :aliases => '-l', :banner => "Generate only what's needed for a gem that will be local to a single application"
    ...
    def gem(name)
      ...
      template(File.join("newgem/LICENSE.txt.tt"),           File.join(target, "LICENSE.txt"),                         opts) unless options[:local]
      template(File.join("newgem/README.md.tt"),             File.join(target, "README.md"),                           opts) unless options[:local]
