...
module Bundler
  class Definition
    ...
    def ensure_equivalent_gemfile_and_lockfile(explicit_flag = false)
      ...
      msg << "\n\nYou have deleted from #{SharedHelpers.gemfile_name}:\n" << deleted.join("\n") if deleted.any?
      msg << "\n\nYou have changed in #{SharedHelpers.gemfile_name}:\n" << changed.join("\n") if changed.any?
      msg << "\n"
    ...
    def validate_ruby!
      ...
      if diff = ruby_version.diff(Bundler::RubyVersion.system)
        ...
        msg = case problem
              when :engine
                "Your Ruby engine is #{actual}, but your #{SharedHelpers.gemfile_name} specified #{expected}"
              when :version
                "Your Ruby version is #{actual}, but your #{SharedHelpers.gemfile_name} specified #{expected}"
              when :engine_version
                "Your #{Bundler::RubyVersion.system.engine} version is #{actual}, but your #{SharedHelpers.gemfile_name} specified #{ruby_version.engine} #{expected}"
              when :patchlevel
                if !expected.is_a?(String)
                  "The Ruby patchlevel in your #{SharedHelpers.gemfile_name} must be a string"
                else
                  "Your Ruby patchlevel is #{actual}, but your #{SharedHelpers.gemfile_name} specified #{expected}"
                end
        end
